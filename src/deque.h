...
template <typename Item> class Deque {
  private:
    ...
    std::size_t front{};
    [[nodiscard]] bool full(const Vector<Item> &vec) { return vec.size() == vec.capacity(); }
  ...
  public:
    ...
    Deque(Deque &&rhs) noexcept : m_data(std::move(rhs.m_data)), front(rhs.front) {
        rhs.front = 0;
    };
    ...
    [[nodiscard]] std::size_t size() const {
        return (m_data.size() - 1) * Vector<Item>::DEFAULT_SIZE + m_data.back().size() - front;
    }

...
template <typename T, typename E> class Result {
    ...
    explicit Result(Ok, T p_val) : m_data(std::in_place_index<0>, std::move(p_val)) {};
    explicit Result(Err, E p_error) : m_data(std::in_place_index<1>, std::move(p_error)) {};
  ...
  public:
    ...
    template <typename F> auto map(F &&f) -> Result<std::invoke_result_t<F, T>, E> {
        if (is_ok()) {

...
class List {
 private:
  struct Node {
    ...
    explicit Node(const Item& val = Item{}, Node* prev = nullptr,
 ...
 public:
  ...
  class iterator : public const_iterator {
   protected:
    iterator(const List<Item>& p_list, Node* p_current)
        : const_iterator(p_list, p_current) {}
    friend class List<Item>;
  ...
  iterator insert(iterator it, const Item& data) {
    ...
    m_size++;
    return {*this, curr->m_prev = curr->m_prev->m_next = gsl::owner<Node*>{
                       new Node{std::move(data), curr->m_prev, curr}}};
  }
  ...
  iterator erase(iterator it) {
    ...
    gsl::owner<Node*> remove =
        it.p_current;  // NOLINT(cppcoreguidelines-owning-memory)
    ++it;
